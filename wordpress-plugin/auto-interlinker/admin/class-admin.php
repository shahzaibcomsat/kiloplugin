...
class Auto_Interlinker_Admin {
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_ai_strip_all_links', array( $this, 'ajax_strip_all_links' ) );
		...
		wp_localize_script(
			...
			array(
				...
				'strings' => array(
					...
					'confirmReprocess' => __( 'This will reprocess all posts and permanently insert interlinks. This may take a while. Continue?', 'auto-interlinker' ),
					'confirmStrip'     => __( 'This will remove ALL auto-inserted interlinks from all posts. Are you sure?', 'auto-interlinker' ),
				),
			)
		);
	}
	...
	/**
	 * AJAX: Reprocess all posts (extract keywords + permanently apply links).
	 */
		...
		wp_send_json_success(
			array(
				'message' => sprintf(
					...
					__( 'Successfully processed %d posts and applied interlinks.', 'auto-interlinker' ),
					$count
				),
				'count'   => $count,
			)
	...
	/**
	 * AJAX: Process a single post (extract keywords + apply links).
	 */
	public function ajax_process_single() {
		...
		$applied = Auto_Interlinker_Engine::apply_links_to_post( $post_id );

		wp_send_json_success(
			array(
				'message'  => sprintf(
					/* translators: 1: keyword count, 2: links applied */
					__( 'Extracted %1$d keywords. Links %2$s.', 'auto-interlinker' ),
					count( $keywords ),
					$applied ? __( 'applied to post', 'auto-interlinker' ) : __( 'already up to date', 'auto-interlinker' )
				),
				'keywords' => $keywords,
				'applied'  => $applied,
			)
	...
	/**
	 * AJAX: Strip all auto-interlinks from all posts.
	 */
	public function ajax_strip_all_links() {
		check_ajax_referer( 'auto_interlinker_nonce', 'nonce' );
		...
		$count = Auto_Interlinker_Post_Processor::strip_all_links();

		wp_send_json_success(
			array(
				'message' => sprintf(
					/* translators: %d: number of posts cleaned */
					__( 'Removed interlinks from %d posts.', 'auto-interlinker' ),
					$count
				),
				'count'   => $count,
			)
